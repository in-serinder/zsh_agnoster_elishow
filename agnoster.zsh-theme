...
prompt_zsh_logo() {
  ...
  prompt_segment '#b99c4d' "sytm"  "🐧 $current_time"
}
...
prompt_dir() {
  dir_DEpermission="red"
  dir_yeppermission="blue"

  current_dir="$(pwd)"
  current_dir_acc=$(stat -c '%A' .)


  if  [ -w $current_dir ]; then
    
    prompt_segment $dir_yeppermission $CURRENT_FG "($current_dir_acc) %~"
  else
    prompt_segment $dir_DEpermission $CURRENT_FG "($current_dir_acc) %~"
  fi
}
